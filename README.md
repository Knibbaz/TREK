...
<picture>
  ...
  <img src="docs/logo-trek-dark.gif" alt="ROUTD" height="96" />
</picture>
...
<img src="https://github.com/mauriceboe/trek-media/releases/download/readme-assets/TREK1.gif" alt="ROUTD — 60-second tour" width="100%" />
...
<picture>
  ...
  <img src="docs/tiles/grid-desktop.svg" alt="ROUTD feature tiles" width="100%" />
</picture>
...
- **MCP** — expose ROUTD to AI assistants via OAuth 2.1
...
Open `http://localhost:3000`. On first boot ROUTD seeds an admin account — if you set `ADMIN_EMAIL`/`ADMIN_PASSWORD` those are used, otherwise the credentials are printed to the container log (`docker logs trek`).
...
ROUTD works as a Progressive Web App — no App Store needed.

1. Open ROUTD in the browser (HTTPS required)
2. **iOS**: Share ▸ *Add to Home Screen*
3. **Android**: Menu ▸ *Install app* (or *Add to Home Screen*)

ROUTD then launches fullscreen with its own icon, just like a native app.
...
For production, put ROUTD behind a TLS-terminating reverse proxy. ROUTD uses WebSockets for real-time sync, so the proxy **must** support WebSocket upgrades on `/ws`.
...
| **Initial setup** | | |
| `ADMIN_EMAIL` | Email for the first admin on initial boot. Must be set together with `ADMIN_PASSWORD`. If either is omitted a random password is printed to the server log. No effect once a user exists. | `admin@routd.local` |
...
ROUTD is [AGPL v3](LICENSE). Self-host freely for personal or internal company use. If you modify and offer ROUTD as a network service to third parties, your modifications must be open-sourced under the same licence.
