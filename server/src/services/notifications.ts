...
const I18N: Record<string, EmailStrings> = {
  en: { footer: 'You received this because you have notifications enabled in ROUTD.', manage: 'Manage preferences in Settings', madeWith: 'Made with', openRoam: 'Open ROUTD' },
  de: { footer: 'Du erhältst diese E-Mail, weil du Benachrichtigungen in ROUTD aktiviert hast.', manage: 'Einstellungen verwalten', madeWith: 'Made with', openRoam: 'ROUTD öffnen' },
  fr: { footer: 'Vous recevez cet e-mail car les notifications sont activées dans ROUTD.', manage: 'Gérer les préférences', madeWith: 'Made with', openRoam: 'Ouvrir ROUTD' },
  es: { footer: 'Recibiste esto porque tienes las notificaciones activadas en ROUTD.', manage: 'Gestionar preferencias', madeWith: 'Made with', openRoam: 'Abrir ROUTD' },
  nl: { footer: 'Je ontvangt dit omdat je meldingen hebt ingeschakeld in ROUTD.', manage: 'Voorkeuren beheren', madeWith: 'Made with', openRoam: 'ROUTD openen' },
  ru: { footer: 'Вы получили это, потому что у вас включены уведомления в ROUTD.', manage: 'Управление настройками', madeWith: 'Made with', openRoam: 'Открыть ROUTD' },
  zh: { footer: '您收到此邮件是因为您在 ROUTD 中启用了通知。', manage: '管理偏好设置', madeWith: 'Made with', openRoam: '打开 ROUTD' },
  'zh-TW': { footer: '您收到這封郵件是因為您在 ROUTD 中啟用了通知。', manage: '管理偏好設定', madeWith: 'Made with', openRoam: '開啟 ROUTD' },
  ar: { footer: 'تلقيت هذا لأنك قمت بتفعيل الإشعارات في ROUTD.', manage: 'إدارة التفضيلات', madeWith: 'Made with', openRoam: 'فتح ROUTD' },
  id: { footer: 'Anda menerima ini karena Anda telah mengaktifkan notifikasi di ROUTD.', manage: 'Kelola preferensi di Pengaturan', madeWith: 'Dibuat dengan', openRoam: 'Buka ROUTD' },
};
...
const EVENT_TEXTS: Record<string, Record<NotifEventType, EventTextFn>> = {
  en: {
    ...
    vacay_invite: p => ({ title: 'Vacay Fusion Invite', body: `${p.actor} invited you to fuse vacation plans. Open ROUTD to accept or decline.` }),
  ...
  de: {
    ...
    vacay_invite: p => ({ title: 'Vacay Fusion-Einladung', body: `${p.actor} hat dich eingeladen, Urlaubspläne zu fusionieren. Öffne ROUTD um anzunehmen oder abzulehnen.` }),
  ...
  fr: {
    ...
    vacay_invite: p => ({ title: 'Invitation Vacay Fusion', body: `${p.actor} vous invite à fusionner les plans de vacances. Ouvrez ROUTD pour accepter ou refuser.` }),
  ...
  es: {
    ...
    vacay_invite: p => ({ title: 'Invitación Vacay Fusion', body: `${p.actor} te invitó a fusionar planes de vacaciones. Abre ROUTD para aceptar o rechazar.` }),
  ...
  nl: {
    ...
    vacay_invite: p => ({ title: 'Vacay Fusion uitnodiging', body: `${p.actor} nodigt je uit om vakantieplannen te fuseren. Open ROUTD om te accepteren of af te wijzen.` }),
  ...
  ru: {
    ...
    vacay_invite: p => ({ title: 'Приглашение Vacay Fusion', body: `${p.actor} приглашает вас объединить планы отпуска. Откройте ROUTD для подтверждения.` }),
  ...
  zh: {
    ...
    vacay_invite: p => ({ title: 'Vacay 融合邀请', body: `${p.actor} 邀请你合并假期计划。打开 ROUTD 接受或拒绝。` }),
  ...
  'zh-TW': {
    ...
    vacay_invite: p => ({ title: 'Vacay 融合邀請', body: `${p.actor} 邀請您合併假期計畫。開啟 ROUTD 以接受或拒絕。` }),
  ...
  ar: {
    ...
    vacay_invite: p => ({ title: 'دعوة دمج الإجازة', body: `${p.actor} يدعوك لدمج خطط الإجازة. افتح ROUTD للقبول أو الرفض.` }),
  ...
  br: {
    ...
    vacay_invite: p => ({ title: 'Convite Vacay Fusion', body: `${p.actor} convidou você para fundir planos de férias. Abra o ROUTD para aceitar ou recusar.` }),
  ...
  cs: {
    ...
    vacay_invite: p => ({ title: 'Pozvánka Vacay Fusion', body: `${p.actor} vás pozval ke spojení dovolenkových plánů. Otevřete ROUTD pro přijetí nebo odmítnutí.` }),
  ...
  hu: {
    ...
    vacay_invite: p => ({ title: 'Vacay Fusion meghívó', body: `${p.actor} meghívott a nyaralási tervek összevonásához. Nyissa meg a ROUTD-ot az elfogadáshoz vagy elutasításhoz.` }),
  ...
  it: {
    ...
    vacay_invite: p => ({ title: 'Invito Vacay Fusion', body: `${p.actor} ti ha invitato a fondere i piani vacanza. Apri ROUTD per accettare o rifiutare.` }),
  ...
  pl: {
    ...
    vacay_invite: p => ({ title: 'Zaproszenie Vacay Fusion', body: `${p.actor} zaprosił Cię do połączenia planów urlopowych. Otwórz ROUTD, aby zaakceptować lub odrzucić.` }),
  ...
  id: {
    ...
    vacay_invite: p => ({ title: 'Undangan Penggabungan Vacay', body: `${p.actor} mengundang Anda untuk menggabungkan rencana liburan. Buka ROUTD untuk menerima atau menolak.` }),
...
const PASSWORD_RESET_I18N: Record<string, PasswordResetStrings> = {
  en: { subject: 'Reset your password', greeting: 'Hi', body: 'We received a request to reset the password for your ROUTD account. Click the button below to set a new password.', ctaIntro: 'Reset password', expiry: 'This link expires in 60 minutes.', ignore: "If you didn't request this, you can safely ignore this email — your password won't change." },
  de: { subject: 'Passwort zurücksetzen', greeting: 'Hallo', body: 'Wir haben eine Anfrage erhalten, das Passwort für dein ROUTD-Konto zurückzusetzen. Klicke auf den Button unten, um ein neues Passwort festzulegen.', ctaIntro: 'Passwort zurücksetzen', expiry: 'Dieser Link ist 60 Minuten gültig.', ignore: 'Wenn du das nicht warst, ignoriere diese E-Mail — dein Passwort bleibt unverändert.' },
  fr: { subject: 'Réinitialisez votre mot de passe', greeting: 'Bonjour', body: 'Nous avons reçu une demande de réinitialisation du mot de passe de votre compte ROUTD. Cliquez sur le bouton ci-dessous pour définir un nouveau mot de passe.', ctaIntro: 'Réinitialiser le mot de passe', expiry: 'Ce lien expire dans 60 minutes.', ignore: "Si vous n'êtes pas à l'origine de cette demande, ignorez cet e-mail — votre mot de passe ne changera pas." },
  es: { subject: 'Restablecer tu contraseña', greeting: 'Hola', body: 'Recibimos una solicitud para restablecer la contraseña de tu cuenta de ROUTD. Haz clic en el botón de abajo para establecer una nueva contraseña.', ctaIntro: 'Restablecer contraseña', expiry: 'Este enlace caduca en 60 minutos.', ignore: 'Si no solicitaste esto, puedes ignorar este correo — tu contraseña no cambiará.' },
  it: { subject: 'Reimposta la tua password', greeting: 'Ciao', body: 'Abbiamo ricevuto una richiesta di reimpostazione della password per il tuo account ROUTD. Clicca il pulsante qui sotto per impostare una nuova password.', ctaIntro: 'Reimposta password', expiry: 'Questo link scade tra 60 minuti.', ignore: 'Se non hai richiesto questa operazione, ignora questa email — la tua password non cambierà.' },
  nl: { subject: 'Reset je wachtwoord', greeting: 'Hallo', body: 'We hebben een verzoek ontvangen om het wachtwoord voor je ROUTD-account te resetten. Klik op de knop hieronder om een nieuw wachtwoord in te stellen.', ctaIntro: 'Wachtwoord resetten', expiry: 'Deze link verloopt over 60 minuten.', ignore: 'Als jij dit niet hebt aangevraagd, kun je deze e-mail negeren — je wachtwoord blijft ongewijzigd.' },
  ru: { subject: 'Сброс пароля', greeting: 'Здравствуйте', body: 'Мы получили запрос на сброс пароля вашего аккаунта ROUTD. Нажмите кнопку ниже, чтобы установить новый пароль.', ctaIntro: 'Сбросить пароль', expiry: 'Ссылка действительна 60 минут.', ignore: 'Если вы не запрашивали сброс — просто проигнорируйте это письмо, пароль останется прежним.' },
  zh: { subject: '重置您的密码', greeting: '您好', body: '我们收到了重置您的 ROUTD 账户密码的请求。点击下方按钮设置新密码。', ctaIntro: '重置密码', expiry: '此链接将在 60 分钟后失效。', ignore: '如果这不是您本人的请求，可以忽略本邮件 — 您的密码不会改变。' },
  'zh-TW': { subject: '重設您的密碼', greeting: '您好', body: '我們收到了重設您 ROUTD 帳號密碼的請求。點擊下方按鈕以設定新密碼。', ctaIntro: '重設密碼', expiry: '此連結將於 60 分鐘後失效。', ignore: '若非您本人發起的請求，請忽略此郵件 — 您的密碼不會變更。' },
  hu: { subject: 'Jelszó visszaállítása', greeting: 'Szia', body: 'Kérést kaptunk a ROUTD-fiókod jelszavának visszaállítására. Kattints az alábbi gombra az új jelszó beállításához.', ctaIntro: 'Jelszó visszaállítása', expiry: 'Ez a link 60 perc után lejár.', ignore: 'Ha nem te kérted ezt, nyugodtan hagyd figyelmen kívül ezt az e-mailt — a jelszavad változatlan marad.' },
  ar: { subject: 'إعادة تعيين كلمة المرور', greeting: 'مرحبا', body: 'تلقينا طلبًا لإعادة تعيين كلمة المرور لحسابك في ROUTD. انقر على الزر أدناه لتعيين كلمة مرور جديدة.', ctaIntro: 'إعادة تعيين كلمة المرور', expiry: 'تنتهي صلاحية هذا الرابط خلال 60 دقيقة.', ignore: 'إذا لم تطلب هذا، يمكنك تجاهل هذه الرسالة — لن تتغير كلمة المرور الخاصة بك.' },
  br: { subject: 'Redefinir sua senha', greeting: 'Olá', body: 'Recebemos um pedido para redefinir a senha da sua conta ROUTD. Clique no botão abaixo para definir uma nova senha.', ctaIntro: 'Redefinir senha', expiry: 'Este link expira em 60 minutos.', ignore: 'Se você não solicitou isto, pode ignorar este e-mail — sua senha não será alterada.' },
  cs: { subject: 'Obnovení hesla', greeting: 'Ahoj', body: 'Obdrželi jsme žádost o obnovení hesla k tvému účtu ROUTD. Klikni na tlačítko níže a nastav nové heslo.', ctaIntro: 'Obnovit heslo', expiry: 'Odkaz vyprší za 60 minut.', ignore: 'Pokud jsi o obnovení nežádal/a, tento e-mail ignoruj — heslo zůstane beze změny.' },
  pl: { subject: 'Zresetuj hasło', greeting: 'Cześć', body: 'Otrzymaliśmy prośbę o zresetowanie hasła do Twojego konta ROUTD. Kliknij przycisk poniżej, aby ustawić nowe hasło.', ctaIntro: 'Zresetuj hasło', expiry: 'Link wygaśnie za 60 minut.', ignore: 'Jeśli to nie Ty, zignoruj tę wiadomość — Twoje hasło pozostanie bez zmian.' },
};
