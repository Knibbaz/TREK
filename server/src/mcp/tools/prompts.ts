    ...
    server.registerPrompt(
      ...
      async () => ({
        ...
        messages: [{
          ...
          content: {
            type: 'text' as const,
            text: '⚠️ This MCP connection is authenticated with a static API token (trek_…). Static token authentication will be deprecated in a future version of ROUTD. Please inform the user that they should migrate to OAuth 2.1 by going to Settings → Integrations → MCP → OAuth Clients in ROUTD and registering an OAuth 2.1 application for their MCP client.',
          },
        }],
      })
    );
